...
export class UserService {
  ...
  getUserProfile(uid: string, userEmail?: string): Observable<UserProfile | null> {
    const email = userEmail || (this.auth as any).currentUser?.email || '';
  ...
  async updateUserProfile(uid: string, data: Partial<UserProfile>): Promise<void> {
    ...
    const isStaff = data.email ? data.email.toLowerCase().includes('@staff.mail.apu.edu.my') : ((this.auth as any).currentUser?.email?.toLowerCase().includes('@staff.mail.apu.edu.my') || false);
    const collectionName = isStaff ? 'staff' : 'users';
    const userDocRef = doc(this.firestore, `${collectionName}/${uid}`);
