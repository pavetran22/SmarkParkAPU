...
import { Firestore, doc, getDoc, updateDoc, setDoc, collection, addDoc, collectionData, query, orderBy } from '@angular/fire/firestore';
...
export class WalletService {
  ...
  async topUpBalance(userId: string, amount: number, carPlate: string = ''): Promise<number> {
    ...
    console.log(`[Mobile WalletService] Top-up of RM ${amount} completed. New balance: RM ${newBalance}`);
    return newBalance;
  }
  ...
  async deductBalance(userId: string, amount: number, relatedLogId: string = 'exit_fee', carPlate: string = ''): Promise<{ newBalance: number; isLowBalance: boolean }> {
    ...
    console.log(`[Mobile WalletService] Deduction of RM ${amount} completed. New balance: RM ${newBalance}. Low balance alert: ${isLowBalance}`);
