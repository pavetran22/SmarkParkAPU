...
@Component({
  ...
  template: `
    <div style="position: relative; display: flex; min-height: 100vh; align-items: center; justify-content: center; padding: 2rem; background: #070a13; overflow: hidden; font-family: 'Inter', sans-serif;">
        ...
        <div class="cctv-hud" style="position: absolute; top: 20px; left: 20px; color: #10b981; font-family: monospace; font-size: 14px; z-index: 2; font-weight: bold; text-shadow: 0 0 4px rgba(16,185,129,0.5);">
            <span class="rec-dot"></span> REC [CAM 02]
        </div>
        ...
        <div class="cctv-hud" style="position: absolute; top: 20px; right: 20px; color: #10b981; font-family: monospace; font-size: 14px; z-index: 2; font-weight: bold; text-shadow: 0 0 4px rgba(16,185,129,0.5);">
            SECURE LINK AUTHORIZED
        </div>

        <!-- Glassmorphism Signup Card -->
        <div class="glass-panel animate-fade-in" style="position: relative; z-index: 3; width: 100%; max-width: 480px; padding: 2.5rem; background: rgba(10, 15, 30, 0.65); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); border-radius: 1.5rem; box-shadow: 0 30px 60px rgba(0, 0, 0, 0.5); border: 1px solid rgba(255, 255, 255, 0.12);">
            <div style="text-align: center; margin-bottom: 2rem;">
                <div style="display: flex; justify-content: center; margin-bottom: 1rem;">
                    <div style="background: rgba(59, 130, 246, 0.1); padding: 16px; border-radius: 50%; color: #3b82f6; border: 1px solid rgba(59, 130, 246, 0.2); box-shadow: 0 0 20px rgba(59, 130, 246, 0.15);">
                        <lucide-icon name="plus-circle" [size]="32"></lucide-icon>
                    </div>
                </div>
                <h1 style="font-size: 1.875rem; font-weight: 800; color: #ffffff; margin: 0; letter-spacing: -0.02em;">Join SmartPark</h1>
                <p style="color: #94a3b8; font-size: 0.9rem; margin-top: 0.5rem; font-weight: 500;">Register your account and vehicle</p>
            </div>

            <form (ngSubmit)="handleSubmit()" style="display: flex; flex-direction: column; gap: 1.25rem;">
                <div *ngIf="error" style="background: rgba(239, 68, 68, 0.15); color: #f87171; padding: 0.75rem; border-radius: 12px; font-size: 0.875rem; font-weight: 600; border: 1px solid rgba(239, 68, 68, 0.3);">
                    {{ error }}
                </div>

                <div class="input-group">
                    <label>Full Name</label>
                    <input class="modern-input" type="text" [(ngModel)]="name" name="name" placeholder="John Doe" />
                </div>
                ...
                <button type="submit" class="submit-btn" [disabled]="loading">
                    {{ loading ? 'Creating Secure Account...' : 'Register Secure Profile' }}
                </button>
            </form>

            <div style="margin-top: 1.5rem; text-align: center; font-size: 0.875rem; color: #94a3b8; font-weight: 500;">
                Already registered? 
                <span (click)="goToLogin()" style="color: #3b82f6; font-weight: 700; cursor: pointer; transition: color 0.2s;" onmouseover="this.style.color='#60a5fa'" onmouseout="this.style.color='#3b82f6'">
                    Sign in
                </span>
            </div>
        </div>
  ...
  styles: [`
    ...
    .submit-btn { width: 100%; background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); color: white; border: none; padding: 14px; border-radius: 12px; font-weight: 800; font-size: 1rem; cursor: pointer; transition: 0.2s; margin-top: 1rem; box-shadow: 0 4px 12px rgba(37, 99, 235, 0.25); }
