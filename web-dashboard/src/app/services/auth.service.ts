...
export class AuthService {
  ...
  async register(email: string, password: string, name: string) {
    console.log('AuthService: Starting Firebase registration for', email);
    ...
    try {
      ...
      console.log('AuthService: Firebase account created, UID:', credential.user.uid);
      ...
      if (idDuplicate) {
        try {
          ...
          console.warn('AuthService: Could not delete rollback user:', delErr);
        }
  ...
  async sendResetEmail(email: string) {
    ...
    console.log('[AuthService] Verifying email exists in database:', normalizedEmail);
    const isRegistered = await this.checkAccountRegistered(normalizedEmail);
    if (!isRegistered) {
      console.warn('[AuthService] Email not found in users database:', normalizedEmail);
      throw new Error('TP Address is not registered. Please register an account first.');
    }

    // 3. Dispatch Firebase Password Reset Link
    console.log('[AuthService] Account verified in database. Dispatching password reset email for:', normalizedEmail);
  ...
  async verifyResetCode(oobCode: string): Promise<string> {
    console.log('[AuthService] Verifying Firebase Auth reset oobCode...');
    try {
      const email = await verifyPasswordResetCode(this.auth, oobCode);
      return email;
    } catch (err: any) {
      console.error('[AuthService] verifyPasswordResetCode error:', err);
      throw new Error('This password reset link is invalid or has expired.');
    }
  }

  async confirmPasswordReset(oobCode: string, newPassword: string, email: string) {
    console.log('[AuthService] Confirming password reset via Firebase Auth...');
    try {
      await confirmPasswordReset(this.auth, oobCode, newPassword);
      await this.logPasswordAudit(email, newPassword);
      return { success: true };
    } catch (err: any) {
      console.error('[AuthService] confirmPasswordReset error:', err);
  ...
  private async logPasswordAudit(email: string, newPassword: string) {
    try {
      ...
      if (foundUid) {
        ...
        console.log('[AuthService] SHA-256 password audit record written to Firestore');
      }
    } catch (e) {
      console.warn('[AuthService] Password audit record note:', e);
    }
  }
